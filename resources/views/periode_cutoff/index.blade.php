<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="">
                <a href="{{ route('periode-cutoff.create') }}"
                            ...
                            <tbody>
                                ...
                                @foreach ($periode_cutoffs as $periode_cutoff)
                                    <tr
                                        class="dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600 bg-white border-b">
                                        <td class="px-6 py-4">
                                            {{ $periode_cutoff->kehadiran_start->format('d F Y') }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $periode_cutoff->kehadiran_end->format('d F Y') }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $periode_cutoff->lembur_start->format('d F Y') }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $periode_cutoff->lembur_end->format('d F Y') }}
                                        </td>
                                        <td class="px-6 py-4">
                                            {{ $periode_cutoff->hari_kerja }}
                                        </td>
                                        <td class="px-6 py-4">
                                        ...
                                        <td class="px-6 py-4 text-center">
                                            ...
                                            <div class="flex flex-wrap justify-center gap-3">
                                                <a href="{{ route('periode-cutoff.edit', $periode_cutoff) }}"
                                                ...
                                                <form id="delete-form-{{ $periode_cutoff->id }}"
                                                    action="{{ route('departements.destroy', $periode_cutoff) }}"
        ...
        <script>
            ...
            function askGenerateSlipGaji(id) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: `{{ route('periode-cutoff.generate-slip-gaji') }}`,
                            type: 'POST',
                            dataType: 'json',
                            data: {
                                id: id,
