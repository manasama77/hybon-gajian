<x-perfect-scrollbar as="nav" aria-label="main" class="flex flex-col flex-1 gap-4 px-3">
    ...
    @if (auth()->user()->hasRole('admin'))
        <x-sidebar.dropdown title="Setup" :active="Str::startsWith(request()->route()->uri(), 'setup')">
            ...
            <x-sidebar.sublink title="Hari Libur" href="{{ route('setup.hari-libur.index') }}" :active="Str::startsWith(request()->route()->uri(), 'setup/hari-libur')" />
        </x-sidebar.dropdown>
    @endif
    ...
    <x-sidebar.link title="Kasbon" href="{{ route('data-kasbon.index') }}" :isActive="Str::startsWith(request()->route()->uri(), 'data-kasbon')">
        <x-slot name="icon">
            <i class="fa-solid fa-hand-holding-dollar text-[1.3rem]" aria-hidden="true"></i>
        </x-slot>
    </x-sidebar.link>
